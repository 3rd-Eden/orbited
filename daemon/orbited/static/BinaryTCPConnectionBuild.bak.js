...
bytesToHex = function(bytes) {
    ...
    for (var i = 0; i < bytes.length; ++i) {
        var byte = bytes[i]
        var hex1 = byte >> 4
        output += base10ToHex[hex1]
        output += base10ToHex[byte - (hex1 << 4)]
    }
    return output
}
// end @include(hex.js)
...
bytesCharValue = function(charBytes) {
        switch(charBytes.length) {
            case 1:
                return charBytes[0];
            case 2:
                return ((charBytes[0] - 192) << 6)  + (charBytes[1] - 128)
            case 3:
                return ((charBytes[0] - 224) << 12) + ((charBytes[1] - 128) << 6) + (charBytes[2] - 128)
            case 4:
                return ((charBytes[0] - 240) << 18) + ((charBytes[1] - 128) << 12) + ((charBytes[1] - 128) << 6) + (charBytes[2] - 128)
        }
}
...
bytesToUTF8 = function(bytes) {
    ...
    while (bytes.length > 0) {
        var charBytes = [bytes.shift()]
        var numBytes = charByteLength(charBytes[0])
        console.log(numBytes + ' length')
        if (bytes.length < numBytes -1)
            throw new Error("Invalid UTF-8 encoded data")
        for ( var i = 0; i < numBytes-1; ++i)
            charBytes.push(bytes.shift())
        var val = bytesCharValue(charBytes)
        console.log('val: ' + val)
