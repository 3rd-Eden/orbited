...
class TestRevolved(object):
    ...
    def test_connect(self):
        ...
        self.csp.trigger_connection_made(conn)
        self.csp.trigger_connection_made(conn2)
    ...
    def test_message(self):
        ...
        conn.add_message_callback(_message_cb)
        # Now trigger a message
        self.csp.trigger_connection_made(conn)
        conn.on_receive_message('[1,"PONG", []]')
    ...
    def test_auth(self):
        ...
        # Anonymous User
        conn.on_receive_message(json.encode([1, "AUTH", ["", ['']]]))
        assert self.sent_msg == json.encode([1, "ERR", ["Not Allowed"]])
        # Invalid User
        conn.on_receive_message(json.encode([1, "AUTH", ['blah', ['']]]))
        assert self.sent_msg == json.encode([1, "ERR", ["Invalid User"]])
        # Invalid Password
        conn.on_receive_message(json.encode([1, "AUTH", ['mario', ['badpass']]]))
        assert self.sent_msg == json.encode([1, "ERR", ["Invalid Password"]])
        # Valid Auth
        conn.on_receive_message(json.encode([1, "AUTH", ['mario', ['mariopass']]]))
    ...
    def test_subscribe(self):
        ...
        conn.on_receive_message(json.encode([1, "AUTH", ['mario', ['mariopass']]]))
        conn.on_receive_message(json.encode([2, "SUBSCRIBE", ['orbited']]))
        assert self.sent_msg == json.encode([2, "OK", [""]])
        conn.on_receive_message(json.encode([3, "SUBSCRIBE", ['not-allowed-channel']]))
    ...
    def test_publish(self):
        ...
        conn.on_receive_message(json.encode([2, "SUBSCRIBE", ['orbited']]))
        assert self.sent_msg == json.encode([2, "OK", [""]])
        conn.on_receive_message(json.encode([3, "PUBLISH", ['orbited', 'hello world']]))
    ...
    def test_send(self):
        ...
        conn.on_receive_message(json.encode([1, "AUTH", ['mario', ['mariopass']]]))
        conn2.on_receive_message(json.encode([1, "AUTH", ['michael', ['michaelpass']]]))
        conn.on_receive_message(json.encode([2, "SUBSCRIBE", ['orbited']]))
        conn.on_receive_message(json.encode([3, "SEND", ['michael', 'hello world']]))
        assert self.dispatcher.sent_messages == [[u'mario', u'michael', u'hello world']]
        conn.on_receive_message(json.encode([3, "SEND", ['unknownrecipient', 'hello world']]))
    ...
    def test_disconnect(self):
        ...
        conn.on_receive_message(json.encode([1, "DISCONNECT", []]))
        conn.on_receive_message(json.encode([1, "AUTH", ['mario', ['mariopass']]]))
        conn2.on_receive_message(json.encode([1, "DISCONNECT", []]))
        assert len(self.revolved.connections) == 0